...
function findBinding(bindings, descriptions) {
  if (!Array.isArray(bindings)) return null

  var expected = Array.isArray(descriptions)
    ? descriptions.map(function(description) { return String(description || "") })
    : [String(descriptions || "")]
  for (var descriptionIndex = 0;
       descriptionIndex < expected.length;
       descriptionIndex++) {
    for (var bindingIndex = 0;
      ...
      if (binding
          && String(binding.description || "") === expected[descriptionIndex])
        return binding
    }
  }
